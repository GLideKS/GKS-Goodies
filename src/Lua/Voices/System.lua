...
/*--------------GOAL SOUNDS---------------*/
		if p.laps >= check_lapcount or p.pflags & PF_FINISHED then
			if GKSR_Voices[mo.skin].victory
			and not mo.victorysoundplayed then
				S_PlayCharVoice(mo, mo.skin, "victory")
				mo.victorysoundplayed = true
			end
		elseif mo.victorysoundplayed then
			mo.victorysoundplayed = false
		end
/*--------------HURRY SOUNDS---------------*/
		if mo.hurry_voice and not mo.hurryplayedsound
		and GKSR_Voices[mo.skin].hurry
		and not (p.laps >= check_lapcount or p.pflags & PF_FINISHED) then
			S_PlayCharVoice(mo, mo.skin, "hurry")
			mo.hurryplayedsound = true
		end
/*--------------READY SOUNDS---------------*/
		if leveltime == 25
		and GKSR_Voices[mo.skin].ready then
			S_PlayCharVoice(mo, mo.skin, "ready", p)
		end
/*--------------GO SOUNDS---------------*/
		if leveltime == 140
		and GKSR_Voices[mo.skin].go then
			S_PlayCharVoice(mo, mo.skin, "go", p)
		end
/*--------------DEFAULT VOICE---------------*/
	else
		if leveltime == 35 then --READY
			S_StartSound(mo, sfx_defred, p)
		end
		if leveltime == 105 then --SET
			S_StartSound(mo, sfx_defset, p)
		end
		if leveltime == 140 then --GO
			S_StartSound(mo, sfx_defgo, p)
		end
	end
end